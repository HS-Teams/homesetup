...
[[ ! ${HHS_ACTIVE_DOTFILES} =~ (^|[[:space:]])bash_commons($|[[:space:]]) ]] &&
...
# @function: Cross-shell read command that works in both Bash and Zsh.
# @param $1..$N [Req] : The read parameters.
function __hhs_read() {
  local zsh_args=() prompt="" arg

  if [[ -n "$ZSH_VERSION" ]]; then
    for arg in "${@}"; do
      case "$arg" in
        -p) prompt=1; continue ;;
        -p*) prompt="${arg#-p}"; continue ;;
        -n) zsh_args+=("-k"); continue ;;
        -n*) zsh_args+=("-k" "${arg#-n}"); continue ;;
        -a) zsh_args+=("-A"); continue ;;
        -*) zsh_args+=("${arg}") ;;
        *)
          if [[ -n "$prompt" ]]; then
            printf '%s' "${arg}";
...
# @function: Cross-shell read array that works in both Bash and Zsh.
# @param $1 [Req] : The name of the array to read into.
function __hhs_read_array() {
  local array_name="$1" temp_line=
  local -a temp_array=()

  [[ -z "$array_name" ]] && {
    echo "__hhs_read_array: Array name required." >&2
    return 1
  }
